...
if($fromPage=='houseMember.php')
{
    $updateId=$indigentApplication->getId();
}

if($fromPage=='HouseholdBudgetInfo.php')
{
...
if($updateId==$indigentApplication->getId())
{
    $readonly="";
}
...
$sql = "select * from tblhouseholdbudget where householdID='$updateId' ";
...
if(isset($check))
{
        ...
        if ($result->num_rows > 0)
        {
            ...
            {
                $bondPayment=$row["bond"];
                $rental=$row["rental"];
                $electricity=$row["electricity"];
                $water=$row["water"];
                $food=$row["food"];
                $transport=$row["transport"];
                $education=$row["educationFee"];
                $medicalExpense=$row["medicalexpense"];
                $otherExpenses=$row["otherExpense"];
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	{
           include './PHP/HouseholdBurdget.php';
           include './PHP/DatabaseManager.php';
           $databaseManager=new DatabaseManager();
           $householdbudget=new HouseholdBurdget($_POST['bondPay'], $_POST['rental'], $_POST['electricity'], $_POST['water'], $_POST['food'], $_POST['transport'], $_POST['education'], $_POST['medExp'], $_POST['otherExp'],$_POST['monIncome']);
           $_SESSION['householdbudget'] = serialize($householdbudget);

           //$databaseManager->updateHouseholdBudget($householdbudget,$updateId);

              //============================================================================================
		   
		    $bondPayment=$_POST['bondPay'];
			$rental=$_POST['rental'];
			...
			$medicalExpense=$_POST['medExp'];
			$otherExpense=$_POST['otherExp'];
			$monIncome=$_POST['monIncome'];
			
			
			
			
		    $sqlCheckId = "SELECT * FROM tblhouseholdbudget WHERE householdID ='$updateId'";
			...
			if(isset($CheckId))
			{
				...
				else 
				{
					$sql = "INSERT INTO tblhouseholdbudget(householdID,bond,rental,electricity,water,food,transport,educationFee,medicalexpense,otherExpense,monthlyIncome)"
						 . " VALUES('$updateId','$bondPayment','$rental','$electricity','$water','$food','$transport','$education','$medicalExpense','$otherExpenses','$monIncome')";
           ...
           //==========================================================================================
            if($updateId==$indigentApplication->getId())
            {
                $sql = "select * from householdmember where addedByID='$updateId' ";
...
<script type="text/javascript" src="ValidateHouseBudget.js"></script>
...
<form name="form3" action="<?php echo $_SERVER['PHP_SELF'] ;?>" method="POST" onsubmit="return validateForm()">
	<section id="register">
        <div class="container">
			...
			<div class="panel panel-info">
                            <div class="panel-heading" > <h4 class="thin text-center"><?php echo 'Household Expenditures for :'.$updateId;?></h4></div>
			  <div class="panel-body">
				
                                <label for="exampleInputName2">Monthly Income R</label><label id="lblmonIncome"></label>
                                <input type="number" class="form-control" id="monIncome" name="monIncome"  required value="<?php print $monthlyIncome; ?>" onkeyup="validateMonIncome()"  >
				
				<label for="exampleInputName2">Bond Payment R</label><label id="lblbondPay"></label>
                                <input type="number" class="form-control" id="bondPay"  onkeyup="validateBond()" name="bondPay" <?php print $readonly?>  required value="<?php print $bondPayment; ?>"  >
				
				<label for="exampleInputName2">Rental R</label> <label id="lblrental"></label>
                                <input type="number" class="form-control" id="rental"  onkeyup="validateRent()" name="rental" <?php print $readonly?>  required value="<?php print $rental; ?>"  >
				
				<label for="exampleInputName2">Electricity R</label> <label id="lblelectricity"></label>
                                <input type="number" class="form-control" id="electricity" onkeyup="validateElectricity()" name="electricity" <?php print $readonly?>  required value="<?php print $electricity; ?>"  >
				
				<label for="exampleInputName2">Water R</label> <label id="lblwater"></label>
                                <input type="number" class="form-control" id="water" onkeyup="validateWater()" name="water" <?php print $readonly?>  required value="<?php print $water; ?>"  >
				
				<label for="exampleInputName2">Food R</label>  <label id="lblfood"></label>
                                <input type="number" class="form-control" id="food" onkeyup="validateFood()" name="food" <?php print $readonly?>  required value="<?php print $food; ?>" >
				
				<label for="exampleInputName2">Transport R</label> <label id="lbltransport"></label>
                                <input type="number" class="form-control" id="transport" onkeyup="validateTransport()" name="transport" required value="<?php print $transport; ?>" >
				
				<label for="exampleInputName2">Education R</label> <label id="lbleducation"></label>
				<input type="number" class="form-control" id="education" onkeyup="validateEducation()" name="education" required value="<?php print $education; ?>">
			
				<label for="exampleInputName2">Medical Expenses R</label> <label id="lblmedExp"></label>
				<input type="number" class="form-control" id="medExp" onkeyup="validatemedExp()" name="medExp" required value="<?php print $medicalExpense; ?>"  >
				
				
				<label for="exampleInputName2">Other Expenses R</label> <label id="lblotherExp"></label>
				<input type="number" class="form-control" id="otherExp" onkeyup="validateOtherExp()"  name="otherExp" required value="<?php print $otherExpenses; ?>" >
				...
				<table>
					...
					<td>
					<center><button type="submit" name="btnSubmit"  id="btnSubmit" class="btn btn-success btn-lg active">Submit</button></center>
					</td>

...
if(isset($check))
{
        ...
        if ($result->num_rows > 0)
        {
            ...
            while($row = $result->fetch_assoc()) 
            {
                ...
                $cellNo=$row['cellno'];
                $homeTell=$row['hometellno'];
                $workTell=$row['worktellno'];
                $email=$row['email'];
...
if(!$isRegistered)
{
    $_SESSION["updateMssg"]="No Contact information found for you, please go back to main page and click apply link and apply...!!";
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
        if(isset($_POST["btnUpdate"]))
	{
            ...
            $homeTell=$_POST['homeTell'];
            $workTell=$_POST['workTell'];
            $email=$_POST['email'];
            ...
            //=================Updating=========================================
               ...
               $sql = "UPDATE tblcontact SET cellno ='$cellNo', hometellno = '$homeTell',worktellno='$workTell' ,email='$email'"
                           . "WHERE id ='$id'";
               
               if($con->query($sql))
               {
                     $_SESSION["updateMssg"]="Your Contact information have been  updated...!!";
                     header('location:updateFeedback.php');
                   
                     $username=password_hash($email, PASSWORD_DEFAULT);
                     $sql="UPDATE tbllogin SET username='$username'  Where id='$id'";
                     $con->query($sql);
               }
               else
               {
...
<head>
    ...
    <![endif]-->
 <script type="text/javascript" src="validateContact.js"></script>
        ...
        <div class="container">
			...
			<div class="panel panel-info">
			  ...
			  <div class="panel-body">
				...
				<label for="exampleInputName2">Cell No</label> <label id="lblCell"></label>
                                <input type="number" class="form-control" id="cellNum" name="cellNum" required onkeyup="validatePhone()"  value="<?php print $cellNo; ?>" placeholder="0729255678" >
				
				<label for="exampleInputName2">Home Tell No</label><label id="lblTell"></label>
                                <input type="number" class="form-control" id="homeTell" name="homeTell" required onkeyup="validateHomeTel()"  value="<?php print $homeTell; ?>" placeholder="01234567890" >
				
				<label for="exampleInputName2">Work Tell No</label><label id="lblworkTell"></label>
                                <input type="number" class="form-control" id="workTell" name="workTell" required onkeyup="validateWorkTel()"  value="<?php print $workTell; ?>" placeholder="01234567890" >
				
				<label for="exampleInputName2">Email Address</label><label id="lblEmail"></label>
                                <input type="email" class="form-control" id="email" name="email" required onkeyup="validateEmail()"  value="<?php print $email; ?>" placeholder="john@gmail.com" " >

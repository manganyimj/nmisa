...
if($_SERVER['REQUEST_METHOD']=='POST')
{
        ...
        if(isset($_POST["btnView"]))
        {
            ...
            if(isset($_POST["updateId"]))
            {
              ...
              if(count($ids)==1)
              {
                 $_SESSION["viewId"]=$ids[0];
                 header('location:ViewDetailInfor.php'); 
              }
              else
              {
                 $_SESSION["viewMssg"]="Please select only one row :".count($ids); 
              }
              /*foreach ($ids as $id)
              {
                  echo 'ID :'.$id."<br>";
            ...
            {
                $_SESSION["viewMssg"]="please Select Row to view..!!";
             
            }
             
             
        }
        else if(isset($_POST["btnAck"]))
        {
            $ids=  array();
            $ids=$_POST["updateId"];
            $count=0;
            ...
            foreach ($ids as $id)
            {
                ...
                $sqlCheckId = "SELECT * FROM acknowledgement WHERE id ='$id'";
                ...
                {
                    $mssg="Your EMM application have been received and we are busy processing it";
            ...
            if($added && $removed)
            {
               $mssg="Acknolegement message(s) sent to the selected applicant(s), however this applicant($removedId) have received acknolegement before as a results their acknolegement was not sent."; 
            }
            else if($added)
            {
                 $mssg="Acknolegement message(s) sent to the selected applicant(s)";
            }
            else
            {
                 $mssg="Acknolegement message(s) was not sent because all the selected applicant(s) have received their acknologement.";
            }
            $_SESSION["cleckMssg"]=$mssg;
            header('location:cleckFeedBack.php');
        }
}
                    ...
                    <div class="panel panel-info">
                         <center>
                              <?php
                                ...
                                if(isset($_SESSION["viewMssg"]))
                                {
                                    echo "<center><P style=color:orange>".$_SESSION["viewMssg"]."</p></center>";
                                }
		   ...
		   <div class="panel-heading" > <h4 class="thin text-center">New Applicant(s)</h4></div>
			  <div class="panel-body">
                             ...
                             <?php
                                ...
                                $sql = "select * from tblindigentapplication,tblapplicationstatus where tblindigentapplication.id = tblapplicationstatus.applicantID AND status='InProgress' ORDER BY date DESC";
                                    ...
                                    if ($result->num_rows > 0)
                                    {
                                        ...
                                        while($row = $result->fetch_assoc()) 
                                        {
                                            echo '<tr>
                                                ...
                                                <td><input type="checkBox" name="updateId[]" id="updateId" value="'.$row["applicantID"].'"></a></td>
                                           </tr>';
                            ...
                            ?>
                              ...
                              if($availableusers==true)
                              {
                                  echo '<center> <table>
                                        ...
                                        <tr>
                                            <td><button type="submit" name="btnView"  id="btnView" class="btn btn-primary btn-lg active small">View Detail Infor</button></td>
                                        <td> <button type="submit" name="btnAck"  id="btnAck" class="btn btn-primary btn-lg active small">Acknowledge Applications</button></td>

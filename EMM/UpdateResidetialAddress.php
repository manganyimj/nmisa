...
//$sql= "SELECT * FROM residentialaddress,tblaccount Where id=$id AND tblaccount.id=residentialaddress.id";

$sql= "SELECT * FROM tblaccount,residentialaddress Where tblaccount.id=residentialaddress.id AND residentialaddress.id=$id ";
...
if(!$isRegistered)
{
    $_SESSION["updateMssg"]="No Application information found for you, please go back to main page and click apply link and apply...!!";
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	if(isset($_POST["btnUpdate"]))
	{
              //=================Updating=========================================
                ...
                $streetName=$_POST['streetNum'];
                $surbub=$_POST['suburb'];
                $city=$_POST['city'];
                $postalCode=$_POST['postalCode'];
                $POBox=$_POST['pobNum'];
               
               $sql = "UPDATE residentialaddress SET wardno ='$wardNum', housenum = '$houseNum',streetno='$streetName' ,suburd='$surbub',city='$city',postalcode='$postalCode',pob_num='$POBox'"
                           . "WHERE id ='$id'";
               
               if($con->query($sql))
               {
                     $_SESSION["updateMssg"]="Your Residential Address  information have been  updated...!!";
                     header('location:updateFeedback.php');
                   
                     $username=password_hash($email, PASSWORD_DEFAULT);
                     $sql="UPDATE tblaccount SET accNum='$accNum'  Where id='$id'";
                     $con->query($sql);
               }
               else
               {
			...
			<div class="panel panel-info">
			  ...
			  <div class="panel-body">
                              ...
                              <label for="exampleInputName2">Account Number</label><label  id="lblAccount"></label>
                                <input type="text" class="form-control" id="accNum" name="accNum" onkeyup="validateAccountNumber()" value="<?php print $accNum; ?>"  required placeholder="123" >
				
				
                              <label for="exampleInputName2">Ward No</label><label  id="lblWardNumber"></label>
                                <input type="text" class="form-control" id="wardNum" name="wardNum" onkeyup="validateWardNum()" value="<?php print $wardNum; ?>"  required placeholder="123" >
				
                                <label for="exampleInputName2">House No</label><label  id="lblHouseNum"></label>
				<input type="text" class="form-control" id="houseNum" onkeyup="validateHouseNum()" name="houseNum"  value="<?php print $houseNum; ?>"  ame="houseNum" required placeholder="123" >
				
                                <label for="exampleInputName2">Street Name</label> <label id="tblStreetMun"></label>
				<input type="text" class="form-control" id="streetNum"  onkeyup="validateStreetNum()" name="streetNum" value="<?php print $streetName; ?>"  required placeholder="Toss Ngilima" >
				
				<label for="exampleInputName2">Suburb</label> <label id="lblSub"></label>
				<select name="suburb"  class="form-control" id="suburb">
                                   <option ><?php print $surbub; ?></option>
				...
				<select name="city"  class="form-control" id="city">
                                 <option ><?php print $city; ?></option>
				  <option >Ekurhuleni</option>
				  
				 
				</select>
                                <label for="exampleInputName2">Postal Code</label> <label id="lblpostcode"></label>
				<input type="number" class="form-control" id="postalCode" onkeyup="validatePostCode()" value="<?php print $postalCode; ?>" required name="postalCode" placeholder="123" >
                                
                                
				<label for="exampleInputName2">P.O.Box No</label><label id="lblPOBox"></label>
                                <input type="number" class="form-control"  onkeyup="validatePOBox()"id="pobNum" value="<?php print $POBox; ?>"  required name="pobNum" placeholder="123" >
